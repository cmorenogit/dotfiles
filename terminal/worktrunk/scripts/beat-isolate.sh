...
4. **Escalera QA LOCAL — solo issues chicos / fixes de motor (A-W)**: `/adlc-qa-ladder` pre-vuelo, peldaños 0-6, 8, 9; el 7 contra el stack local queda PARCIAL. Features (A-F): sin escalera. Preparación (el `qa-start.sh` que cita el comando no existe): BD con `source .beat/q.sh`, Node 22 (`node -v`), evidencia en `~/.cache/qa-ladder/<ID>/<fecha>/`, informe en `issues/<ID>/qa-ladder-<fecha>.md` del vault. Es read-only: lo que encuentre → subPR de fix.
...
6. **Revisión sobre el preview**: `/pr-review <PR>` del PR de promoción UNA vez sobre el HEAD actual. **A-W:** además `/adlc-qa-ladder <PR> --issue <ID>` (Convergence Mode: revalida 7, 4 y 3; peldaño 7 = flujo real verificado en la BD del preview con `preview_db.py`; peldaño 8 REUTILIZA ese `/pr-review` si el HEAD no cambió). Veredicto contra el ISSUE, bloqueante para el borrador. **A-F:** sin escalera; `beat-promote --check` + smoke y directo a Nicole y Hakeem.

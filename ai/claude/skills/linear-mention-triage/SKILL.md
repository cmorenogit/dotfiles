...
| 10 | **Idioma y voz humana — español mexicano neutral, profesional y HUMANO** | todos | hay localismos (de cualquier región), o el texto suena robótico/mecánico/telegráfico en vez de como lo escribiría una persona competente y clara (ver Rules · Idioma) |
...
**Estilo (borradores).** Confiado y lean: sin disclaimers, sin auto-aclaración de lane, sin hedging ("debería confirmar…"), sin deletrear next-steps asumidos. Liderar con el veredicto, luego la evidencia precisa. **Lean no es seco:** cortar el relleno (hedging, disclaimers) NUNCA significa cortar la calidez ni el contexto que ayuda a entender. La respuesta debe sonar **humana y clara**, como la escribiría un colega competente — no como salida de máquina.

**Idioma y voz humana (borradores — Criterio 10 del gate, anillo 2).** Todo borrador que se publica en Linear va en **español mexicano neutral, profesional y humano**.
- **Registro:** oficina mexicana sin coloquialismo. "tú" (nunca "vos"), sin localismos de NINGUNA región (ni rioplatenses —"acá", "che", "recién" temporal—, ni de otras), sin spanglish evitable (preferir español salvo término técnico sin equivalente: "deploy", "merge", "feature flag").
- **Humanización (lo que pidió César):** que se lea como lo escribiría una persona, no una máquina. Profesional **no** es acartonado, telegráfico ni robótico. Está bien una línea de contexto, un anexo o un detalle si hace la respuesta **más entendible** — la claridad humana gana sobre la economía de palabras. El objetivo es que el equipo entienda sin esfuerzo y sienta que hay una persona del otro lado.
- **Anillo 2** del gate (`pi`/gpt-5.5, solo GAUGE pesado): revisa registro + naturalidad porque un solo modelo no detecta bien sus propios localismos ni su propio "tono robot". El prompt a `pi` lleva el borrador + el check corto "¿español mexicano neutral, profesional y humano —no robótico—, sin localismos?" y devuelve PASS / qué falla.
- **Nota:** aplica a los **borradores de Linear** (output al equipo), no a la comunicación con César (que sigue su preferencia de siempre).

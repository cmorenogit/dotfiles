...
**Gate (sub-agente independiente + salida estructurada).** Ningún borrador se muestra sin pasar el **gate §3.5 del triaje** (8 criterios), corrido por un evaluador independiente que devuelve por criterio `{pass|fail|N/A, evidencia}`. Ajustes que PRIMAN sobre el texto del triaje:
...
- **Criterio 9** (solicitud de review bien formada — la cura de RYR-111, 11/06): una pedida de review formal SOLO se atiende si (a) el **comentario de la solicitud** menciona a **@juli** (QA siguiente) y a **@ignacio** (revisión/merge), y (b) el issue está en **`In Review`**. Pedida inválida → NO se emite veredicto; se responde devolución de proceso señalando qué falta. El estado lo valida ADEMÁS el hook (`linear-write-guard.sh` v2: API en vivo si hay `LINEAR_API_KEY`, si no snapshot `~/.claude/cache/linear-state/{ID}.json` que persiste el pre-flight de `respond`; fail-closed → ⚠️ en el permission prompt). Aplica solo a casos de review.
...
| @juli + @ignacio mencionados en la SOLICITUD de review | 2 | prosa: criterio 9 del gate (el evaluador re-lee el comentario de la pedida; el hook lo recuerda en el prompt) |

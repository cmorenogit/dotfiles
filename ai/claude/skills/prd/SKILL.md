---
name: prd
description: Generate a complete PRD from Google Drive docs for SDD workflow. Trigger when user asks to generate, create, write, or update a PRD (Product Requirements Document) — `/prd [name]`, "generar PRD", "crear PRD", "escribir requisitos del producto".
---
...
### PHASE 4.5: Save versioned copy to vault
...
**Path:** `_work/apprecio/projects/{slug}/prds/{change-name}-v{N}.md`

Where `N` is determined by:
1. List existing files matching `_work/apprecio/projects/{slug}/prds/{change-name}-v*.md`
2. If none → `N = 1`
3. If present → `N = max(existing) + 1`

**File content** = full PRD markdown body, with this vault frontmatter prepended:

```yaml
---
type: PRD
status: Draft
project: <slug>                              # rr | fuerza | sl | engagement | app | _shared
...
If `_work/apprecio/projects/{slug}/prds/` doesn't exist, create it.
...
- Vault: _work/apprecio/projects/{slug}/prds/{change-name}-v{N}.md
- User stories: {count}
- Requirements: {count}
- Source docs: {count}

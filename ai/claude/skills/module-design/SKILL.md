...
## 13. Testing Strategy (MANDATORY ADR)

**This section is MANDATORY — it is the Test Pyramid & Structure ADR that governs how the module is tested. `module-design-review` validates this section explicitly. `sdd-tasks` consumes the ratio target to generate test-related tasks. `sdd-verify` enforces the ratio gate at phase boundaries.**
...
### 13.0 Test Pyramid Target (REQUIRED — declare explicit ratios)

Declare the target distribution for this module. Default is **60% unit / 30% service / 10% E2E** unless the project's stack has an established alternative. Tolerance for verify gate: **±10%**.

| Tier | Target % | What goes here | Real Postgres? |
|------|---------|----------------|-----------------|
| Unit | 60% | Pure business logic (validations, calculations, state machines) — no I/O | No (fake/in-memory) |
| Service | 30% | Full service flows with real Postgres + RLS + auth | **YES — mandatory** |
| E2E / Integration | 10% | User journeys across app + edge function + DB | Real stack |
| (Contract / Security) | overhead | Type-drift, boundary, RLS matrix, observability contracts | As needed |

Contract and security tests are **overhead** on top of the pyramid, not a tier of their own.

### 13.1 Pattern Reference (REQUIRED — auto-detect)

Identify the **most established module of the current stack** as the reference pattern. Mirror its directory structure, naming conventions, and fixture patterns. Do NOT invent a new structure.

For a Deno/Supabase stack:
- Scan `supabase/functions/*/__tests__/` and count tests per module.
- Pick the module with the richest test suite AND the canonical `unit/ + service/` structure as the reference.
- Record the chosen reference in this ADR with a one-line rationale.

**Pattern Reference for this module:** {auto-detected — e.g., `challenge-api/__tests__/` has 105 service tests + `unit/service/` structure, chosen as reference}

### 13.2 Directory Structure (MANDATORY — mirror the reference)

```
supabase/functions/{module-api}/__tests__/
├── fixtures/           # Reusable test data with defaults
├── helpers/            # Mock client, table operations, user pool
├── unit/
│   ├── schemas/       # Zod schema validation
│   ├── services/      # Business logic
│   └── security/      # Auth, tenant isolation, permissions (fake DB OK)
├── service/            # Full flow tests with REAL Postgres
├── contracts/          # Type-drift, observability, boundary (overhead)
└── integration/        # End-to-end across systems (optional)
```

The `service/` directory is **not optional**. If a module has zero service/ tests, the verify gate fails (ratio 0% vs target 30%).

### 13.3 Layer Definitions

| Layer | Scope | Tool | When Written | Surface |
|-------|-------|------|-------------|---------|
| Unit | Business logic (validations, calculations, rules) — no I/O | Deno.test + assertions from std | Together with code in same task | Backend |
| Service | Full flows against **REAL Postgres** (grant → transaction → budget) | Deno.test + local stack 44321 | Separate task per endpoint — **never in unit/** | Backend |
...
| Contract | Type-drift, observability, boundary | Deno.test | Dedicated task | Backend |

### 13.4 Anti-shortcut Rules (apply in tasks + apply + verify)

When `sdd-tasks` generates testing tasks and `sdd-apply` writes tests, these rules MUST be encoded in acceptance criteria:

1. **Service tier MUST use real Postgres** (port 44321 test stack). Fake-supabase fixtures are ONLY for `unit/` tier. Never use fake-supabase to simulate service-level flows.
2. **Auth user cleanup in `finally` blocks** (Luisa 2026-03-18 standard) for every test that creates auth users. Prevents GoTrue saturation (~430 users).
3. **No timing-dependent tests**: no `setTimeout`, no `sleep(ms)` waiting for state, no polling loops with arbitrary timeouts. Use deterministic waits (listen for events, poll fixed DB state).
4. **Assertions MUST be specific**: no empty `.toBeDefined()` / `.toBeTruthy()`. Assert exact error codes (RF_ERROR_CODES style), exact response shapes, exact DB state.
5. **No test interdependence**: each test generates its own UUIDs (`crypto.randomUUID()`), seeds its own fixtures, cleans up in `finally`. Order-independent.
6. **Mirror pattern reference, don't invent**: read 2-3 service tests from the reference module before writing. Adopt patterns that work; flag anti-patterns you want to avoid.
7. **One logical assertion per test**: a test with 10 `expect()` is 10 tests. Split.

...
### Step 4e: Pyramid Ratio Gate (consumes MDD section 13 if present)

**Purpose**: catch test-pyramid drift early. If the MDD declares a Test Pyramid Target (section 13.0), compute the actual ratio from the test suite and compare. If no MDD/section, behave as informational only (graceful fallback).

```
Retrieve `sdd/{change-name}/module-design` from Engram (if exists).

IF MDD exists AND has "Test Pyramid Target" section:
├── Read target ratios (e.g., unit=60, service=30, e2e=10)
├── Read tolerance (default ±10 percentage points)
├── Count tests per tier in the change scope:
│   ├── unit/     → count Deno.test in __tests__/unit/**
│   ├── service/  → count Deno.test in __tests__/service/**
│   ├── e2e/ or integration/ → count in __tests__/integration/** or __tests__/e2e/**
│   └── contracts/, security/ → overhead, not in pyramid
├── Compute actual percentages (excluding overhead)
├── Compare actual vs target:
│   ├── If |actual - target| <= tolerance for every tier → PASS
│   ├── If any tier deviates more than tolerance → CRITICAL (block verify)
│   └── Emit per-tier table with target · actual · delta · verdict
└── Special case: if service tier actual = 0% and service target > 0%,
    always CRITICAL regardless of tolerance (service tier is mandatory).

IF MDD exists BUT no Test Pyramid section:
├── Compute actual ratios (informational)
├── Flag WARNING: "MDD exists but lacks Test Pyramid ADR — cannot enforce ratio"
└── Do NOT fail the verify.

IF no MDD:
├── Compute actual ratios (informational)
├── Report: "No MDD for this change — ratio gate inactive (informational only)"
└── Do NOT fail the verify.
```
...
```
Pyramid Ratio Gate (vs MDD target)
| Tier    | Target | Actual | Delta | Verdict |
|---------|--------|--------|-------|---------|
| unit    | 60%    | 62%    | +2    | ✅ OK   |
| service | 30%    | 0%     | -30   | ❌ CRITICAL (service tier empty) |
| e2e     | 10%    | 23%    | +13   | ⚠ WARN  |

Gate verdict: FAIL — service tier empty.
```
